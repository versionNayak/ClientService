...
public class BrokerageMasterFeedBOThread implements Runnable {

	private static Logger log = LoggerFactory.getLogger(BrokerageMasterFeedBOThread.class);
	...
	private final Integer PRIMARY_KEY = 255;
	...
	public void run() {
		try {
			...
			if (name.contains(FinexaBOColumnConstant.DBF_LOWER_CASE) || name.contains(FinexaBOColumnConstant.DBF_UPPER_CASE)) {
				...
				uploadResponseDTO = readExcelFeed(brokerageMasterBODTO, uploadResponseDTO);
			}
			if (uploadResponseDTO.isStatus()) {
				String test = rejectedRowNumbers+FinexaBOColumnConstant.BROKERAGE_RECORD_REJECTION_MESSAGE;
				int reasonLength=rejectedRowNumbers.length()+(FinexaBOColumnConstant.BROKERAGE_RECORD_REJECTION_MESSAGE).length();
				//System.out.println("for testing "+reasonLength+","+test.length());
				...
				if (noOfRejectedRecords > 0) {
					...
					if (reasonLength <= ROW_LENGTH) {
						...
						backOfficeUploadHistory.setReasonOfRejection(
								FinexaBOColumnConstant.BROKERAGE_RECORD_REJECTION_MESSAGE + rejectedRowNumbers);

					} else {
						rejectedRowNumbers = rejectedRowNumbers.substring(0, rejectedRowNumbers.indexOf(','))
						/* + rejectedRowNumber.toString() + "," */;

						backOfficeUploadHistory.setReasonOfRejection(FinexaBOColumnConstant.BROKERAGE_RECORD_REJECTION_MESSAGE
			...
			log.debug("BROKERAGE FEED UPLOADING FINISHED!");
			...
			if (allRowsInserted == true) {
				...
				backOfficeUploadHistory
						.setReasonOfRejection(FinexaBOColumnConstant.BROKERAGE_RECORD_REJECTION_MESSAGE + rejectedRowNumbers);
				backOfficeUploadHistory = uploadHistoryRepo.save(backOfficeUploadHistory);
			} else {
				backOfficeUploadHistory.setStatus(FinexaBOColumnConstant.STATUS_REJECTED);
	 ...
	 */
	private UploadResponseDTO readExcelFeed(BrokerageMasterBODTO brokerageMasterBODTO, UploadResponseDTO uploadResponseDTO)
		...
		try {
			...
			// file
			databaseColumnIDAndNameList = masterDatabaseColumnNamesBORepository.getIDAndDatabaseColumnNames(FinexaBOColumnConstant.FEED_TYPE_BROKM);

			// Fetching the the database column IDs and feed column names list based on the
			// type of feed file
			feedColumnNameList = masterFeedColumnNamesBORepository.getIDAndFeedColumnNames(FinexaBOColumnConstant.FEED_TYPE_BROKM);
			...
			for (String dbCOlumnID : FinexaBOColumnConstant.BROKERAGE_DB_COLUMN_ID_LIST) {
				try {
				...
				try {
					...
					if (batchCount == 1000) {
						...
						log.debug("LIST size() when COUNTER is: " + counter + ", and batchCount is: "
								+ brokerageMasterBOList.size());
						uploadResponseDTO = saveModelData(brokerageMasterBOList, uploadResponseDTO);
						brokerageMasterBOList = new ArrayList<BrokerageMasterBO>();
						batchCount = 0;

						brokerageMasterBOList = populateModelList(feed, brokerageMasterBODTO, brokerageMasterBOList, advisorUser);
	...
	private List<BrokerageMasterBO> populateModelList(Map<String, String> feedData, BrokerageMasterBODTO brokerageMasterBODTO,
			List<BrokerageMasterBO> brokerageMasterBOList, AdvisorUser advisorUser) throws ParseException {
		// TODO Auto-generated method stub

		try {
			boolean isRecordInvalid = false;

			if (feedData.get(FinexaBOColumnConstant.BROKERAGE_TRANSACTION_NUMBER) == null
					|| feedData.get(FinexaBOColumnConstant.BROKERAGE_TRANSACTION_NUMBER).toString().trim().isEmpty()
					|| feedData.get(FinexaBOColumnConstant.BROKERAGE_TRANSACTION_NUMBER).toString().trim() == "") {
				isRecordInvalid = true;
			} else if (feedData.get(FinexaBOColumnConstant.BROKERAGE_TRANSACTION_DATE) == null
					|| feedData.get(FinexaBOColumnConstant.BROKERAGE_TRANSACTION_DATE).toString().trim().isEmpty()
					|| feedData.get(FinexaBOColumnConstant.BROKERAGE_TRANSACTION_DATE).toString().trim() == "") {
				isRecordInvalid = true;
			} else if (feedData.get(FinexaBOColumnConstant.BROKERAGE_PROCESS_DATE) == null
					|| feedData.get(FinexaBOColumnConstant.BROKERAGE_PROCESS_DATE).toString().trim().isEmpty()
					|| feedData.get(FinexaBOColumnConstant.BROKERAGE_PROCESS_DATE).toString().trim() == "") {
			...
			brokerageMasterBO
					.setSchemeRTACode(feedData.get(FinexaBOColumnConstant.BROKERAGE_SCHEME_RTA_CODE));
			// brokerageMasterBO.setSchemeName(feedData.get("Fund Description"));
			brokerageMasterBO.setAmcCode(feedData.get(FinexaBOColumnConstant.BROKERAGE_AMC_CODE));
			brokerageMasterBO.setFolioNumber(
					formatNumbers(feedData.get(FinexaBOColumnConstant.BROKERAGE_FOLIO_NUMBER)));
			brokerageMasterBO.setInvestorName(feedData.get(FinexaBOColumnConstant.BROKERAGE_INVESTOR_NAME));
			brokerageMasterBO
					.setTransactionType(feedData.get(FinexaBOColumnConstant.BROKERAGE_TRANSACTION_TYPE));
			//System.out.println("F"+feedData.get(FinexaBOColumnConstant.BROKERAGE_FROM_DATE));
			brokerageMasterBO.setFromDate(formatUtilDateToSQLDate(dateFormatter(feedData.get(FinexaBOColumnConstant.BROKERAGE_FROM_DATE))));
			//System.out.println("TO"+feedData.get(FinexaBOColumnConstant.BROKERAGE_TO_DATE));
			brokerageMasterBO.setToDate(formatUtilDateToSQLDate(dateFormatter(feedData.get(FinexaBOColumnConstant.BROKERAGE_TO_DATE))));
			//System.out.println("T"+feedData.get(FinexaBOColumnConstant.BROKERAGE_TRANSACTION_DATE));
			brokerageMasterBOPK.setTransactionDate(formatUtilDateToSQLDate(dateFormatter(feedData.get(FinexaBOColumnConstant.BROKERAGE_TRANSACTION_DATE))));
			//System.out.println("P"+feedData.get(FinexaBOColumnConstant.BROKERAGE_PROCESS_DATE));
			brokerageMasterBOPK.setProcessDate(formatUtilDateToSQLDate(dateFormatter(feedData.get(FinexaBOColumnConstant.BROKERAGE_PROCESS_DATE))));
			//System.out.println("AMOUNT:"+feedData.get(FinexaBOColumnConstant.BROKERAGE_AMOUNT));
			brokerageMasterBO.setAmount(formatAmount(feedData.get(FinexaBOColumnConstant.BROKERAGE_AMOUNT)));
			brokerageMasterBO.setBrokeragePercentage(
					feedData.get(FinexaBOColumnConstant.BROKERAGE_BROKERAGE_PERCENTAGE));
			//System.out.println("BROKEARGE AMOUNT:"+feedData.get(FinexaBOColumnConstant.BROKERAGE_BROKERAGE_AMOUNT));
			brokerageMasterBO.setBrokerageAmount(
					formatAmount(feedData.get(FinexaBOColumnConstant.BROKERAGE_BROKERAGE_AMOUNT))
							);
			brokerageMasterBO
					.setBrokerageType(feedData.get(FinexaBOColumnConstant.BROKERAGE_BROKERAGE_TYPE));
			brokerageMasterBOPK.setTransactionNumber(
					formatNumbers(feedData.get(FinexaBOColumnConstant.BROKERAGE_TRANSACTION_NUMBER))
							);

			if(brokerageMasterBODTO.getNameRTA() == FinexaBOColumnConstant.RTA_TYPE_FRANKLIN)
				brokerageMasterBO.setAmcCode(FinexaBOColumnConstant.AMC_FRANKLIN);
			else if(brokerageMasterBODTO.getNameRTA() == FinexaBOColumnConstant.RTA_TYPE_SUNDARAM)
				brokerageMasterBO.setAmcCode(FinexaBOColumnConstant.AMC_SUNDARAM);
			else
				brokerageMasterBO.setAmcCode(feedData.get(FinexaBOColumnConstant.BROKERAGE_AMC_CODE));

			brokerageMasterBO.setLookupRtabo(lookupRTABOMap.get(brokerageMasterBODTO.getNameRTA()));
			brokerageMasterBO.setAdvisorUser(advisorUser);
	...
	private UploadResponseDTO readDBFFeed(BrokerageMasterBODTO brokerageMasterBODTO, UploadResponseDTO uploadResponseDTO)
			throws IOException, InvalidFormatException, ParseException {
		try {
			InputStream initialStream = brokerageMasterBODTO.getNameSelectFile()[0].getInputStream();
...
*/
			...
			while (rowObjects2 != null) {
				try {
					...
					if (batchCount == 1000) {
						counter++;
						log.debug("brokerageMasterBOList size() when COUNTER is:" + counter + " batchCount is: "
								+ brokerageMasterBOList.size());
						uploadResponseDTO = saveModelData(brokerageMasterBOList, uploadResponseDTO);
						brokerageMasterBOList = new ArrayList<BrokerageMasterBO>();
						batchCount = 0;
						...
						brokerageMasterBOList = populateModelList(databaseColumnIDAndDataMap, brokerageMasterBODTO,
			...
			if (batchCount > 0) {
				log.debug("brokerageMasterBOList size() before LAST: " + brokerageMasterBOList.size());
				uploadResponseDTO = saveModelData(brokerageMasterBOList, uploadResponseDTO);
			}
	...
	private synchronized UploadResponseDTO saveModelData(List<BrokerageMasterBO> brokerageMasterBOList,
			UploadResponseDTO uploadResponseDTO) {
		// TODO Auto-generated method stub
		//System.out.println("Uploading AUM");
		try {
			if (brokerageMasterBOList != null && brokerageMasterBOList.size() > 0) {
				brokerageMasterBORepository.save(brokerageMasterBOList);
